...
// NOTE DÉPLOIEMENT : Décommenter ces imports lors de l’intégration Vite
// import EspaceAbonne, { createSubscriberProfile, PILIERS_ANNUELS } from “./EdenSubscriber”;
// import EdenDemo from “./EdenDemo”;

// ═══════════════════════════════════════════════════════════════════════════
// EDEN ACADÉMIE — Application Unifiée v2.0 (MODE TEST)
// Bilan 360° · Portrait Eaux·Os·Chair · Espace Abonné (3 niveaux)
// Institut de Leadership Familial — Fondé par Zady Zozoro
//
// MODE TEST ACTIF :
//   Si VITE_ANTHROPIC_KEY est définie dans Vercel → appel direct Anthropic
//   Si non → appel via backend Render (/api/…)
//   Sans aucune clé → rapport de démonstration (app fonctionne quand même)
// ═══════════════════════════════════════════════════════════════════════════

// ─── FONCTION UNIVERSELLE D’APPEL IA ────────────────────────────────────────
// Vite remplace import.meta.env.VITE_ANTHROPIC_KEY au moment du build
// Si la clé est dans Vercel → appel direct Anthropic (mode test)
// Si non → appel via backend Render (mode production)
const VITE_ANTHROPIC_KEY = import.meta.env.VITE_ANTHROPIC_KEY || “”;

async function edenAI({ prompt, systemPrompt, history, endpoint = “diagnostic” }) {
// ── MODE TEST : clé dans les variables Vercel ────────────────────────────
if (VITE_ANTHROPIC_KEY) {
const messages = history && history.length > 0
? history.map(m => ({ role: m.role, content: m.content }))
: [{ role: “user”, content: prompt }];
const body = {
model: “claude-sonnet-4-6”,
max_tokens: endpoint === “portrait” ? 2500 : 3000,
messages,
};
...
// ── MODE PRODUCTION : via backend Render ─────────────────────────────────
const apiPath = endpoint === “portrait” ? “/api/portrait-report” : “/api/diagnostic-report”;
...
// ── DÉMO GUIDÉE ──
if (currentModule === “demo”) {
// EdenDemo est importé depuis EdenDemo.jsx
// Décommenter l’import ligne 3 pour activer en production Vite
if (typeof EdenDemo !== “undefined”) {
return <EdenDemo onBack={() => setCurrentModule(“home”)} />;
}
// Fallback si EdenDemo pas encore importé
return (
<div className="eden-app">
<div className="eden-header">
<div className="eden-logo">Académie Eden</div>
<div className="eden-title">Démonstration</div>
</div>
<div className="eden-wrap">
<div className="section">
<div style={{ fontSize:9, color:C.gold, letterSpacing:”.22em”, textTransform:“uppercase”, marginBottom:12 }}>◈ Module Démo</div>
<div style={{ fontFamily:”‘Cormorant Garamond’,serif”, fontSize:20, color:”#F0EBE0”, marginBottom:12 }}>Le module démo est prêt</div>
<p style={{ fontSize:13, color:C.muted, lineHeight:1.8, marginBottom:20 }}>
Placez <strong style={{color:C.gold}}>EdenDemo.jsx</strong> dans <code style={{color:C.gold}}>src/</code> et décommentez l’import ligne 3 de App.jsx.
</p>
<button className=“btn-secondary” onClick={() => setCurrentModule(“home”)}>← Retour</button>
</div>
</div>
</div>
);
...
const [currentModule, setCurrentModule] = useState(“home”); // home | bilan | portrait | admin | admin_login | espace_abonne | demo
...
{/* Bouton Démo principal */}
<div style={{ background:“linear-gradient(135deg,#080C14,#0D1018)”, border:“1px solid #C9A84C33”, padding:“20px”, marginBottom:12 }}>
<div style={{ fontSize:9, letterSpacing:”.22em”, textTransform:“uppercase”, color:C.gold, marginBottom:8 }}>◈ Découverte</div>
<div style={{ fontFamily:”‘Cormorant Garamond’,serif”, fontSize:18, color:”#F0EBE0”, marginBottom:8 }}>Voir une analyse type avant d’acheter</div>
<div style={{ fontSize:12, color:C.muted, lineHeight:1.7, marginBottom:14 }}>Parcourez une démonstration complète — rapport, patterns, plan d’action — sur un cas réel fictif adapté à votre situation.</div>
<button style={{ background:C.gold, border:“none”, color:”#0B0F1A”, padding:“13px 20px”, fontFamily:”‘Jost’,sans-serif”, fontSize:12, fontWeight:700, cursor:“pointer”, width:“100%”, letterSpacing:”.04em” }} onClick={() => setCurrentModule(“demo”)}>
Voir la démonstration gratuite →
</button>
</div>
{/* Admin */}
<div style={{ display:“flex”, gap:8, marginTop:8 }}>
<button className=“btn-secondary” style={{ flex:1, fontSize:10 }} onClick={() => setCurrentModule(“admin_login”)}>Administration</button>
<button className=“btn-secondary” style={{ flex:1, fontSize:10 }} onClick={() => { setAbonnementLevel(“premium”); setCurrentModule(“bilan”); }}>Test Premium</button>
</div>
</div>
<Footer />
</div>
